...
function Service() {
  ...
  useEffect(() =>{
    Voting_Contract().then(async (method)=>{
      await method.view_total_Proposal().call().then((result)=>{
        SetData(result)
      })
    })
  },[])
            ...
            {Data.map((a,index)=>{
              return(
                ...
                <h3 className="Proposal_title">{a.title}</h3>
                 <div className="Proposal_name">주최자 : {a.Starter_user}</div>
                 <div className="Proposal_time">종료시간 : {Unix_timestamp(a.end_time)}</div>
                 <div className="Proposal_button_div">
                   <button className="Proposal_button" onClick={()=>{
                     Vote_Now(a.id)
                   }}>Vote NoW</button>
